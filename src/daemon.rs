...
pub fn transcribe_status(json_output: bool) -> Result<()> {
    ...
    if json_output {
        println!("{}", serde_json::to_string_pretty(&response)?);
    } else if let Some(preview) = response.transcript_preview.as_deref() {
        println!(
...
mod tests {
    ...
    fn synthetic_transcribe_stop_returns_transcript_and_clears_state() {
        ...
        unsafe {
            std::env::set_var("OMNI_CONFIG_FILE", &config_path);
        }
        ...
        crate::config::save_config(&config).expect("config should save");
